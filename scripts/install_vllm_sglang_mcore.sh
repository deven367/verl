...
uv pip install --no-cache "vllm==0.11.0"
...
wget -nv https://github.com/Dao-AILab/flash-attention/releases/download/v2.8.1/flash_attn-2.8.1+cu12torch2.8cxx11abiFALSE-cp312-cp312-linux_x86_64.whl && \
    uv pip install --no-cache flash_attn-2.8.1+cu12torch2.8cxx11abiFALSE-cp312-cp312-linux_x86_64.whl
...
if [ $USE_MEGATRON -eq 1 ]; then
    ...
    NVTE_FRAMEWORK=pytorch uv pip3 install --no-deps git+https://github.com/NVIDIA/TransformerEngine.git@v2.6
    uv pip3 install --no-deps git+https://github.com/NVIDIA/Megatron-LM.git@core_v0.13.1
fi
